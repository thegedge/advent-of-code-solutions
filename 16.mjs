import { max, minBy } from "lodash-es";
...
const solve = (valves, time, n) => {
  ...
  const combos = Array.from(combinations(flowValves, n));

  const solve_ = (peeps, rate, available) => {
    // If we've already opened every flow-y valve, or there's no time left for anyone, the best we can do is the current rate
    if (available == 0 || peeps.every((v) => v[1] <= 1)) {
      return rate;
    }

    // Always move the peep that has more time left
    const peep = minBy(peeps, (v) => v[1]);
    const valve = valves[peep[0]];

    let best = rate;
    if (!valve.open) {
      valve.open = true;
      rate += (peep[1] - 1) * valve.rate;

      for (const { index } of flowValves) {
        if (index == valve.index) {
          continue;
        }

        peep[0] = index;
        peep[1] -= 1 + valve.distances[index];

        // Recurse under the scenario that we open the current valve and travel to the given flow valve. Since we're
        // opening this valve, we flag it as such and then add the new flow rate. We take 1 minute away for opening
        // the valve, then the distance to the destination valve.
        best = Math.max(best, solve_(peeps, rate, available - 1));
  ...
  return max(
    combos.map((combo) => {
      const peeps = combo.map(({ index }) => [index, time - valves[0].distances[index]]);
